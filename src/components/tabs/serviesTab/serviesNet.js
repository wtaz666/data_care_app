...
class ServiesNet extends Component {
    constructor(props) {
        super(props);
        this.state = {
            value1: null,
    ...
    getbarData() {
        axios.get('/AppIsNewController/appConTableNewApp', {
            params: {
                ...
                hostId: sessionStorage.getItem('NetItemId'),
    ...
    getData() {
        axios.get('/resOnline/hostOnlineCount', {
            params: {
                webTimeType: this.state.time
            }
        }).then(res => {
            let data = res.data
            let hostCount = data.hostCount
            let hostOnlineCount = data.hostOnlineCount
            let stylehostOnlineCount = hostOnlineCount / hostCount * 100 + '%';
            let stylehostCount = (1 - (hostOnlineCount / hostCount)) * 100 + '%';
            $('.bottom_box').css('width', stylehostOnlineCount)
            $('.bottom_box1').css('width', stylehostCount)
            this.setState({
                hostOnlineCount: hostOnlineCount,
                hostCount: hostCount,
            })
        })
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (this.props.NetItemId !== nextProps.NetItemId) {
            this.setState({
                clickTab: 0,
                clickTab2: 0,
                timeId: 6,
    ...
    render() {
        const { rankData, lineFlag,tabs,rankId, normSele, normInd, newNormInd, curKpiName, modal1, OnlineTimeData, tableColumns, tableData, clickTab, clickTab2, newTitle, tabs2, lineData } = this.state;
                            ...
                            return <div style={{ height: '100%', paddingTop: '10px' }} key={index}>
                                <div className='dialogbg'>
                                    ...
                                    <div className="choice_box ">
                                        <div className="top_change">{this.state.hostOnlineCount}</div>
                                        <div className="bg_bar">
                                            <div className="inner_change" ></div>
                                        </div>
                                        <div className="bottom_change">{this.state.hostCount}</div>
                                    </div>
                                </div>
                                <div className='dialogbg'>
