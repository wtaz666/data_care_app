...
class NetTab extends Component {
    ...
    render() {
        const { networkData, NetItemId } = this.props;
        const list1 = networkData.map((item) => {
            if (item.id == NetItemId) {
                return item;
            }
        })
        const { tabs, rankId,tabs2, clickTab, clickTab2, tableColumns, tableData, lineData, normSele, normInd, newNormInd, curKpiName, netData, modal1, rankData } = this.state;
