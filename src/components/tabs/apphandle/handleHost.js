...
class HandleHost extends Component {
    constructor(props) {
        super(props);
        this.state = {
            modal1: false,
    ...
    // 单位和数据
    businessAxios() {
        axios.get('http://192.168.0.144:8080/kpiDing/hostDetailPerformanceTable', {
            params: {
        ...
        axios.get('http://192.168.0.144:8080/kpiDing/hostDetailPacketRateTable', {
            params: {
                webTimeType: this.state.timeId,
                pageSize: 10000,
                hostId: this.props.id
    ...
    // 响应时延
    getdashed() {
        axios.get('http://192.168.0.144:8080/kpiDing/hostDetailPerformance', {
            params: {
                hostId: this.props.id,
    ...
    render() {
        const { tabs, tabs2, normSele, normInd, newNormInd, curKpiName, modal1, tableColumns, tableData, FormanceData, clickTab, newTitle, lineData, clickTab2, lineFlag, distributionData } = this.state;
                            ...
                            return <div style={{ height: '100%', paddingTop: '10px' }} key={index}>
                                <div className='dialogbg'>
                                    ...
                                    <div className='xiangyinglv'>
                                        <ul><li><p>建立连接成功率</p><span>{FormanceData ? FormanceData.createRate : ''}</span></li>
                                            <li><p>保持连接成功率</p><span>{FormanceData ? FormanceData.keepAlivedRate : ''}</span></li>
                                            <li><p>关闭连接成功率</p><span>{FormanceData ? FormanceData.clostRate : ''}</span></li>
                                        </ul>
                                    </div>
                                </div>
                                <div className='dialogbg'>
