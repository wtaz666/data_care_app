...
class HandleApp extends Component {
    ...
    // 响应时延
    getdashed() {
        axios.get('http://192.168.0.144:8080/ScreenMonitor/getSingleResponseTime', {
            params: {
                appSourceDataId: this.props.id,
    ...
    render() {
        const { tabs, tabs2, normSele, normInd, newNormInd, curKpiName, modal1, tableColumns, tableData, longTime, newTitle, clickTab, lineData, clickTab2, lineFlag, distributionData } = this.state;
                        ...
                        tabs.map((item, index) => {
                            return <div style={{ height: '100%', paddingTop: '10px' }} key={index}>
                                <div className='dialogbg'>
                                    ...
                                    <i></i>
                                    <div className="shiyan">
                                        <p className='flexStyle'><span>响应时延</span>{
                                            longTime.responseTime ? <span>{longTime.responseTime.value}{longTime.responseTime.measure}</span> : ''
                                        }</p>
                                        <div className='chartBox'>
                                            <div style={{ width: longTime.responseTime ? longTime.responseTime.value > 0 ? longTime.responseTime.value / longTime.max * 100 + '%' : '0%' : '0%' }}></div>
                                        </div>
                                    </div>
                                </div>
                                <div className='dialogbg'>
